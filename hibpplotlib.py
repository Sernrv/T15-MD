...
def plot_scan_one_subplot(traj_list, geom, Ebeam, Btor, Ipl, ax=None, axes='XY',
    ...
    if ax is None:
        plt.figure()
        ax = plt.gca()
        set_axes_param(ax, '%s, м' % axes[0], '%s, м' % axes[1], fontsize=fontsize_axes)
    ...
    for tr in traj_list:
        if tr.Ebeam == Ebeam:
            ...
            if plot_analyzer and hasattr(tr, 'RV_sec_slit'):
                ...
                for i in range(n_slits):
                    for sec_tr in tr.RV_sec_slit[i]:
                        ax.plot(sec_tr[:, index1], sec_tr[:, index2], color=colors[i])
    ...
    if plot_det_line:
        ax.plot(det_line[:, index1], det_line[:, index2], '--o', color=color_sec)
...
def plot_grid_simple(traj_list, geom, Btor, Ipl, onlyE=False,
                     linestyle_A2='--', linestyle_E='-',
                     marker_A2='*', marker_E='p', ax=None, legend_on=True, title_on=True, **kwargs):
    '''
...
def plot_sec_angles(traj_list, Btor, Ipl, Ebeam='all', linestyle='-o'):
    ...
    set_axes_param(ax1, 'UA3 (kV)', r'Exit $\alpha$ (grad)')
    set_axes_param(ax2, 'UA3 (kV)', r'Exit $\beta$ (grad)')
